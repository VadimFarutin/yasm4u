...
public class SQLNaturalJoinRoutine implements Routine {
    @Override
            ...
            for(int j = i + 1; j < state.length; j++) {
                if (SQLRef.class.isAssignableFrom(state[i].type())
                    ...
                    if (intersection.isEmpty()
                            || intersection.containsAll(firstColumns)
                            || intersection.containsAll(secondColumns)) {
                        continue;
                    }
                    ...
                    variants.add(new Joba.Stub(new Ref[]{first, second}, new Ref[]{outputRef}) {
                        ...
                        public void run() {
                            ...
                            try {
                                ...
                                env.naturalJoin(fromFirst.getTable(), fromSecond.getTable(), to.getTable(), union);
